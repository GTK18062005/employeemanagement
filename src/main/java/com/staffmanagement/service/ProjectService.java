...
public class ProjectService {
    ...
    // ✅ EMPLOYEE methods (no @PreAuthorize needed)
    public List<Project> getUserProjects(String username) {
        // Returns projects where user is manager OR team member
        return projectRepository.findByAssignedUser(username);
    }
    ...
    // ✅ MANAGER/ADMIN methods (protected)
    @PreAuthorize("hasRole('PROJECT_MANAGER') or hasRole('ADMIN')")
    public Project createProject(Project project) {
        // Auto-set projectManager to current user
        project.setProjectManager(getCurrentUsername());
    ...
    public ProjectTask createTask(ProjectTask task) {
        // Auto-set assignedBy to current manager
        task.setAssignedBy(getCurrentUsername());
    ...
    // ✅ EMPLOYEE can update own task status
    ...
    public List<Project> getProjectsByManager(String projectManager) {
        // If you want "by manager", use findByProjectManager
    ...
    public ProjectStats getProjectStats(Long projectId) {
        ...
        if (stats.getTotalTasks() > 0) {
            stats.setCompletionPercentage(
                    (stats.getCompletedTasks() * 100) / stats.getTotalTasks()
            );
        }
